#!/usr/bin/env python3
"""
CentOS-Compatible Port Scan Generator
"""
...
class CentOSPortScanGenerator:
    """Generate port scan compatible with CentOS"""
    ...
    def generate_attack(self):
        """Generate port scan"""
        info(f'*** Generating port scan (CentOS)\n')
        ...
        # Generate port scan with nmap
        info('*** Launching port scan (IPv4)\n')
        ...
        scanner.cmd(f'timeout 10 nmap -4 -sS -p 21,22,23,25,80,443,3306,3389,8080 {target.IP()} > /dev/null 2>&1')
        
        info('  Scanning high ports...\n')
        scanner.cmd(f'timeout 10 nmap -4 -p 8000-9000 --max-rate 500 {target.IP()} > /dev/null 2>&1')
...
def main():
    parser = argparse.ArgumentParser(description='Generate port scan (CentOS compatible)')
    ...
    generator = CentOSPortScanGenerator(args.output, args.samples)
    generator.generate_attack()
    
    print("\n" + "="*60)
    print("PORT SCAN PCAP GENERATED (CentOS)")
    print("="*60)
