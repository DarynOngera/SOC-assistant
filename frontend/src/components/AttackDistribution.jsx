    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-2 text-gray-600">Loading attack distribution...</span>
        </div>
      </div>
    );
  }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg">
          ...
          <p className="text-sm">Threat Score: {data.threatScore}</p>
          <div className="mt-2">
            <p className="text-xs font-medium">Severity Breakdown:</p>
            <div className="text-xs space-y-1">
              <div className="flex justify-between">
                <span>Critical:</span>
                <span className="text-red-600">{data.severityBreakdown.critical}</span>
              </div>
              <div className="flex justify-between">
                <span>High:</span>
                <span className="text-orange-600">{data.severityBreakdown.high}</span>
              </div>
              <div className="flex justify-between">
                <span>Medium:</span>
                <span className="text-yellow-600">{data.severityBreakdown.medium}</span>
              </div>
              <div className="flex justify-between">
                <span>Low:</span>
                <span className="text-green-600">{data.severityBreakdown.low}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <h4 className="text-md font-semibold text-gray-900">Top Threats</h4>
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-2xl font-bold text-gray-900">{distributionData.total_attacks}</div>
          <div className="text-sm text-gray-600">Total Attacks</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-2xl font-bold text-gray-900">{Object.keys(distributionData.distribution).length}</div>
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="text-sm text-gray-600">Top Threat</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-2xl font-bold text-gray-900">{distributionData.time_range}</div>
